...
# Join team colors
team_avg <- team_avg %>%
  left_join(teams %>% select(team_abbr, team_color), by = c("team" = "team_abbr"))

ggplot(team_avg, aes(x = reorder(team, avg_sharpe), y = avg_sharpe)) +
  geom_col(aes(fill = team_color), width = 0.7, show.legend = FALSE) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray40") +
  scale_fill_identity() +
  coord_flip() +
  labs(
...
```{r draft-curve-lines, fig.height=7, fig.width=10}
# Line chart: each position's relative Sharpe across rounds — the actual "curve"
curve_lines <- relative_sharpe %>%
...
ggplot(curve_lines, aes(x = tier, y = relative, color = pos_group, group = pos_group)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  geom_line(linewidth = 1.2) +
  geom_point(size = 3.5) +
  geom_text(
    data = curve_lines %>% filter(tier == "Rounds 4-7"),
    aes(label = pos_group), hjust = -0.3, fontface = "bold", size = 4, show.legend = FALSE
  ) +
  scale_color_manual(values = pos_colors[key_positions]) +
  scale_x_discrete(expand = expansion(mult = c(0.05, 0.15))) +
  labs(
    title = "The Draft Curve: How Positional Value Shifts Across Rounds",
    subtitle = "Sharpe relative to round average | Above the line = better bet than average for that round",
    x = NULL, y = "Relative Sharpe (vs. Round Avg)", color = NULL
  ) +
  theme_minimal(base_size = 14) +
  theme(
