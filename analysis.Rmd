...
ggplot(team_avg, aes(x = reorder(team, avg_sharpe), y = avg_sharpe)) +
  ...
  geom_nfl_wordmarks(aes(team_abbr = team), width = 0.065) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray40") +
  scale_fill_manual(values = c("TRUE" = "#2196F3", "FALSE" = "#F44336")) +
  coord_flip() +
  labs(
    title = "Average Round 1 Sharpe Ratio by Team (2018–2022)",
    subtitle = "How well has each team allocated first-round capital? | Min 2 R1 picks",
...
## The Draft Curve: Best Positional Bets by Round

The team chart shows *who* gets it right in Round 1. But teams draft in every round — and they have to make the pick. The question isn't "should you draft at all in Round 3?" — it's "**given that you're picking in Round 3, which positions give you the best relative return?**"
...
```{r draft-curve-bar, fig.height=7, fig.width=11}
# Grouped bar: show the actual vs relative-optimal allocation for key positions
# "Optimal" = allocate proportionally to positions with above-average Sharpe for that round
...
optimal_relative <- relative_sharpe %>%
  filter(pos_group %in% key_positions) %>%
  mutate(weight = pmax(relative, 0)) %>%
  group_by(tier) %>%
  mutate(optimal_pct = ifelse(sum(weight) > 0, weight / sum(weight), 1 / n())) %>%
  ungroup() %>%
  select(pos_group, tier, optimal_pct)

comparison <- actual_alloc %>%
  left_join(optimal_relative, by = c("pos_group", "tier")) %>%
  select(tier, pos_group, actual_pct, optimal_pct) %>%
  pivot_longer(cols = c(actual_pct, optimal_pct), names_to = "type", values_to = "pct") %>%
  mutate(
    type = ifelse(type == "actual_pct", "How Teams Actually Draft", "Sharpe-Optimal for Round"),
...
ggplot(comparison, aes(x = pos_group, y = pct, fill = type)) +
  ...
  scale_fill_manual(values = c(
    ...
    "Sharpe-Optimal for Round" = "#2196F3"
  )) +
  labs(
    title = "How the NFL Drafts vs. How It Should (Round-Adjusted)",
    subtitle = "Actual vs. optimal allocation based on relative Sharpe within each round | Key positions, 2010–2022",
...
cat(paste0(
  "**The heatmap tells the real story.** In Round 1, QB, EDGE, and OT are significantly above the round ",
  ...
  "The draft curve shows that teams systematically overdraft certain positions early and underdraft ",
