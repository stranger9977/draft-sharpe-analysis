...
cat(paste0(
  ...
  "The market dictates the raw contractual value of these players. Teams are more willing to part ",
  "with starting RB talent than they are for WR talent. I have a theory that the mid to low end of ",
  "the WR market is inefficient due to the top end of the WR market pulling up those contracts, but ",
...
```{r eagles-core-data}
# Eagles key players — drafted + acquired
...
# Also grab Mailata (late round pick)
phi_mailata <- player_lookup %>%
  filter(grepl("Mailata", pfr_player_name))

# FA/trade acquisitions
phi_fa_names <- c("Saquon Barkley", "A.J. Brown")
phi_fa <- player_lookup %>%
  filter(grepl(paste(phi_fa_names, collapse = "|"), pfr_player_name)) %>%
  arrange(desc(player_return))

# Combine all key Eagles pieces
phi_all <- bind_rows(
  phi_drafted,
  phi_mailata %>% filter(!pfr_player_name %in% phi_drafted$pfr_player_name),
  phi_fa
) %>%
...
phi_table <- phi_all %>%
  ...
  select(pfr_player_name, pos_group, season, pick, channel, rookie_surplus, second_apy_cap_pct, player_return)
...
phi_gt <- phi_table %>%
  ...
  tab_footnote(
    footnote = "Player return for FA/Trade acquisitions is from when they were originally drafted.",
    locations = cells_column_labels(columns = player_return)
  ) %>%
  tab_options(table.font.size = px(13), table.width = pct(100))

gtsave(phi_gt, filename = "charts/eagles_core.png", vwidth = 900)
