...
### Monotonicity: more rookie-deal hits, more wins

Using the same hit definition from earlier in the article — a player who plays at least **75% of a positional starter's snap share over their first four seasons** — we counted hits per team-season across the active rookie-deal class (drafts Y–4 to Y–1) and split them into three buckets: QB hits, premium non-QB hits (OT / EDGE / WR), and non-premium hits (everything else). Team-seasons are then bucketed into quartiles by hit count within each category.

```{r sharpe-winning-deciles, fig.height=6, fig.width=12}
cat_specs <- tibble::tribble(
  ~cat_col,               ~cat_label,
  "qb_hits",              "QB hits",
  "premium_non_qb_hits",  "Premium non-QB hits\n(OT / EDGE / WR)",
  "non_premium_hits",     "Non-premium hits\n(everything else)"
)

sw_cat_raw <- purrr::map_dfr(cat_specs$cat_col, function(col) {
  # Group by hit count directly (small integers). Collapse sparse tails.
  sw_panel %>%
    mutate(val = .data[[col]]) %>%
    group_by(val) %>%
    summarise(
  ...
  left_join(cat_specs, by = "cat_col") %>%
  mutate(
...
sw_cat_plot <- sw_cat %>%
  # Keep rows with enough support per bucket (avoid tiny-n noise at the tails).
  group_by(cat_label, val) %>% filter(first(n) >= 10) %>% ungroup()

ggplot(sw_cat_plot, aes(x = val, y = value, color = metric, group = metric)) +
  ...
  scale_color_manual(values = c("Win %" = "#08519c", "Playoff appearance rate" = "#b45f06"),
                     name = NULL) +
  scale_size_continuous(range = c(2, 6), name = "n team-seasons") +
  scale_y_continuous(labels = scales::percent, limits = c(0.15, 0.85)) +
  scale_x_continuous(breaks = function(r) seq(floor(r[1]), ceiling(r[2]), 1)) +
  facet_wrap(~ cat_label, nrow = 1, scales = "free_x") +
  labs(
    title = "Where do draft hits actually translate to wins?",
    subtitle = paste0("Hits per team-season, across the active rookie-deal class (drafts Y–4 to Y–1) | 2014–2025, n = ",
                      nrow(sw_panel),
                      ". Buckets with <10 team-seasons dropped."),
    x = "Hits in category (active rookie-deal class)",
    y = NULL,
    caption = "Hit = ≥75% of positional-starter snap share over first 4 seasons | Dashed line = .500"
  ) +
...
gap_one <- function(col) {
  d <- sw_panel %>% mutate(val = .data[[col]])
  zero <- d %>% filter(val == 0) %>% summarise(wp = mean(win_pct), po = mean(made_po), n = n())
  top <- d %>% filter(val >= quantile(val, 0.9, na.rm = TRUE)) %>%
    summarise(wp = mean(win_pct), po = mean(made_po), n = n())
  list(zero = zero, top = top)
}
qb_g <- gap_one("qb_hits")
pnq_g <- gap_one("premium_non_qb_hits")

cat(paste0(
  "The QB panel is the clearest: team-seasons with **zero QB hits** on the active rookie class win just **",
  scales::percent(qb_g$zero$wp, accuracy = 0.1),
  "** of their games and make the playoffs **",
  scales::percent(qb_g$zero$po, accuracy = 1),
  "** of the time, while team-seasons in the top decile of QB hits win **",
  scales::percent(qb_g$top$wp, accuracy = 0.1),
  "** and reach the playoffs **", scales::percent(qb_g$top$po, accuracy = 1),
  "** of the time. Premium non-QB hits show a smaller but still positive slope (**",
  scales::percent(pnq_g$zero$wp, accuracy = 0.1), "** → **",
  scales::percent(pnq_g$top$wp, accuracy = 0.1),
  "**). Non-premium hits barely move the needle — hitting on day-three picks is nice but rarely decides seasons."
))
```
...
- **Realized surplus — actually hitting on picks — is what translates to winning.** Teams in the top quartile win meaningfully more than teams in the bottom quartile, and the playoff-rate gap is even larger.
...
cat(paste0(
  ...
  "7. **Draft hits actually translate to wins — especially at QB.** Across ", nrow(sw_panel),
  " team-seasons from 2014 to 2025, QB hits on the active rookie class move the needle most: ",
  "zero QB hits → **", scales::percent(qb_g$zero$wp, accuracy = 0.1), "** win / **",
  scales::percent(qb_g$zero$po, accuracy = 1), "** playoff rate, top-decile QB hits → **",
  scales::percent(qb_g$top$wp, accuracy = 0.1), "** win / **",
  scales::percent(qb_g$top$po, accuracy = 1), "** playoff rate.\n\n",
