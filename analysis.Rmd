...
We sorted every team-season into quintiles by total rookie-deal surplus and plotted win% and playoff rate by quintile.

```{r sharpe-winning-deciles, fig.height=5, fig.width=10}
sw_quintile_raw <- sw_panel %>%
  mutate(bin = dplyr::ntile(total_surplus, 5)) %>%
  group_by(bin) %>%
  summarise(
    win_pct = mean(win_pct),
    po_rate = mean(made_po),
    min_s = min(total_surplus),
    max_s = max(total_surplus),
    n = n(),
    .groups = "drop"
  ) %>%
  mutate(
...
ggplot(sw_quintile, aes(x = bin, y = value, color = metric, group = metric)) +
  ...
  scale_color_manual(values = c("Win %" = "#08519c", "Playoff appearance rate" = "#b45f06"),
                     name = NULL) +
  scale_y_continuous(labels = scales::percent, limits = c(0.15, 0.7)) +
  scale_x_continuous(breaks = 1:5, labels = sw_quintile_raw$bin_label) +
  labs(
    title = "More draft surplus → more wins and more playoff berths",
    subtitle = paste0("Team-seasons bucketed into quintiles by rookie-deal surplus (drafts Y–4 to Y–1) | 2014–2025, n = ",
                      nrow(sw_panel)),
    x = "Quintile of rookie-deal surplus",
    y = NULL,
...
bot <- sw_panel %>% mutate(b = dplyr::ntile(total_surplus, 5)) %>% filter(b == 1) %>%
  summarise(wp = mean(win_pct), po = mean(made_po))
top <- sw_panel %>% mutate(b = dplyr::ntile(total_surplus, 5)) %>% filter(b == 5) %>%
  summarise(wp = mean(win_pct), po = mean(made_po))

cat(paste0(
  "The pattern is monotonic: teams in the **top quintile** of rookie-deal surplus ",
...
- **Realized surplus — actually hitting on picks — is what translates to winning.** Teams in the top quintile win meaningfully more than teams in the bottom quintile, and the playoff-rate gap is even larger.
...
cat(paste0(
  ...
  "** (p < 0.0001). The top quintile of surplus wins **", scales::percent(top$wp, accuracy = 0.1),
  "** and reaches the playoffs **", scales::percent(top$po, accuracy = 1),
  "** of the time; the bottom quintile wins **", scales::percent(bot$wp, accuracy = 0.1),
