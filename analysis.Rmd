...
A quasi-Sharpe ratio for NFL draft picks: a risk-adjusted measure of what each position is
worth at each draft slot, built on rookie-contract surplus, second-contract value, and
supply-controlled free-agent replacement costs. What the framework says:

- **QB** produces the strongest risk-adjusted returns in the top 10.
- **OT and EDGE** peak in the late first round, not the top 10.
- **WR** offers the best value in Round 2.
- **RB** is negative across every tier in this framework: cheap free-agent alternatives and a low second-contract ceiling.
- **Rookie-contract surplus** (starter production at a fraction of free-agent cost for four years) drives most of a pick's total return, not the second contract.
- **Premium-position surplus translates to wins.** Across 384 team-seasons (2014–2025), teams in the top quintile of premium-position surplus on their active rookie class win 54% and make the playoffs 50%; the bottom quintile wins 48% and makes it 39%.
- **Caveats** (see Assumptions): snap share is an imperfect proxy for production, the model ignores team need, class depth, and individual prospect talent, and some position/tier cells are thin.
...
Analytic intuition says that spending premium draft capital on a "non-premium" position like RB is a horrible idea. On the other side, others cite Love's elite prospect profile and the impact a running back like Saquon Barkley can have on a team's Super Bowl run (see the [Eagles' Super Bowl LIX victory in 2024](https://www.dazn.com/en-US/news/american-football/super-bowl-lix-saquon-barkley-breaks-nfl-record-to-as-philadelphia-eagles-dominate-kansas-city-chiefs-in-first-half/12vr71rrd1txd1em5r9hbxmqh3)).
...
cat(paste0(
  "Five of seven became starters and the hit rate (",
...
non_prem_summary %>%
  filter(pos_group != "DL") %>%
  gt() %>%
  cols_hide(columns = next_positions) %>%
  cols_label(
  ...
  tab_spanner(label = "Next Premium Player Taken",
              columns = c(cf_avg_pr, cf_hit_rate)) %>%
  tab_header(
...
cat(paste0(
  ...
  "free-agent replacement cost, compounded over four years. Consider the 2021 draft: Kyle Pitts ",
  "(pick 4, TE) earned ~4.2% of the cap on his rookie deal, while a replacement TE costs ~5%, ",
  "a savings of roughly $2–3M/yr, or about **$8–12M over four years**. Ja'Marr Chase (pick 5, WR) ",
  "earned ~4.1%, but a replacement WR costs ~10%, a savings of roughly $15M/yr, or about **$60M over ",
  "four years**. Both were legitimate hits on the field, but Chase's four-year rookie surplus dwarfs ",
  "Pitts' purely because WR has a more expensive free-agent market than TE. Same draft, adjacent picks, ",
  "similar production profiles early on, wildly different surplus value, because the cost-controlled ",
...
OT and EDGE peak in the late first round instead of the top 10.
...
early_r2_wr <- df_eligible %>% filter(pos_group == "WR", tier == "Round 2") %>%
  slice_max(player_return, n = 10) %>% summarise(med = median(player_return)) %>% pull(med)
...
cat(paste0(
  ...
  ", or roughly **$", round(best_rb_return$player_return * 275), "M** of value at the 2026 cap) ",
  "is outpaced by an early-Round-2 WR outcome. Taking the median of the top 10 Round 2 WR returns gives us a baseline for an early-R2 hit (",
  round(early_r2_wr, 3), ", ~**$", round(early_r2_wr * 275), "M**). The *best* RB outcome in our sample doesn't match a *typical* early-R2 WR hit. ",
...
# Ensure two distinct worst positions in R1 for the narrative
worst_pair <- unique(most_overdrafted$pos_group)[1:2]
...
cat(paste0(
  ...
  "is the most favorable. Similarly, OT remains an above-average bet deep into the draft.\n\n",
...
# Normalize team codes across PFR-style (player_lookup) and nflfastR-style (schedules),
# plus collapse relocation codes so each franchise is one row
...
build_team_feats <- function(target_year, lookback = 4) {
  ...
  sw_base %>% filter(season %in% dw) %>% group_by(team) %>% summarise(
    ...
    qb_hits = sum(ifelse(pos_group == "QB", is_hit, 0), na.rm = TRUE),
    premium_non_qb_hits = sum(ifelse(pos_group %in% premium_pos_set & pos_group != "QB", is_hit, 0), na.rm = TRUE),
    non_premium_hits = sum(ifelse(!pos_group %in% premium_pos_set, is_hit, 0), na.rm = TRUE),
...
### Monotonicity: more premium-position surplus, more wins

Team-seasons bucketed into quintiles by **premium-position surplus** — the combined rookie-deal surplus generated by QB, OT, EDGE, and WR picks on the active rookie class (drafts Y–4 to Y–1). These are the positions where the Sharpe ratio says expected surplus is highest; this chart asks whether teams that actually *realize* that surplus also win more.

```{r sharpe-winning-deciles, fig.height=5, fig.width=10}
sw_panel <- sw_panel %>%
  mutate(premium_surplus = qb_surplus + premium_non_qb_surplus)

sw_quintile_raw <- sw_panel %>%
  mutate(bin = dplyr::ntile(premium_surplus, 5)) %>%
  group_by(bin) %>%
  summarise(
    win_pct = mean(win_pct),
    po_rate = mean(made_po),
    min_s = min(premium_surplus),
    max_s = max(premium_surplus),
    n = n(),
    .groups = "drop"
  ) %>%
  mutate(
    bin_label = paste0(
      c("Bottom 20%", "Q2", "Q3", "Q4", "Top 20%"),
      "\n(", round(min_s, 1), "–", round(max_s, 1), ")"
    )
  )

sw_quintile <- sw_quintile_raw %>%
  tidyr::pivot_longer(c(win_pct, po_rate), names_to = "metric", values_to = "value") %>%
  mutate(metric = dplyr::recode(metric,
    win_pct = "Win %",
    po_rate = "Playoff appearance rate"
  ),
  metric = factor(metric, levels = c("Win %", "Playoff appearance rate")))

ggplot(sw_quintile, aes(x = bin, y = value, color = metric, group = metric)) +
  geom_hline(yintercept = 0.5, linetype = "dashed", color = "grey60") +
  geom_line(linewidth = 1.4) +
  geom_point(size = 4) +
  geom_text(aes(label = scales::percent(value, accuracy = 1)),
            vjust = -1.2, size = 3.6, fontface = "bold", show.legend = FALSE) +
  scale_color_manual(values = c("Win %" = "#08519c", "Playoff appearance rate" = "#b45f06"),
                     name = NULL) +
  scale_y_continuous(labels = scales::percent, limits = c(0.30, 0.60)) +
  scale_x_continuous(breaks = 1:5, labels = sw_quintile_raw$bin_label) +
  labs(
    title = "More premium-position surplus → more wins and more playoff berths",
    subtitle = paste0("Team-seasons bucketed into quintiles by premium-position surplus (QB / OT / EDGE / WR, drafts Y–4 to Y–1) | 2014–2025, n = ",
                      nrow(sw_panel)),
    x = "Quintile of premium-position surplus (cap % pts)",
    y = NULL,
...
bot <- sw_panel %>% mutate(b = dplyr::ntile(premium_surplus, 5)) %>% filter(b == 1) %>%
  summarise(wp = mean(win_pct), po = mean(made_po))
top <- sw_panel %>% mutate(b = dplyr::ntile(premium_surplus, 5)) %>% filter(b == 5) %>%
  summarise(wp = mean(win_pct), po = mean(made_po))

cat(paste0(
  "The pattern is monotonic: teams in the **top quintile** of premium-position surplus ",
  "win **", scales::percent(top$wp, accuracy = 0.1),
  "** of their games and make the playoffs **", scales::percent(top$po, accuracy = 1),
  "** of the time. Teams in the **bottom quintile** win **",
  scales::percent(bot$wp, accuracy = 0.1),
  "** and reach the playoffs just **", scales::percent(bot$po, accuracy = 1),
  "**. That is a **", round((top$wp - bot$wp) * 100, 1),
  "-point** gap in win percentage and a **",
  round((top$po - bot$po) * 100, 0),
  "-point** gap in playoff rate, purely as a function of how much surplus value a team's active ",
  "rookie class has generated at premium positions."
))
```
...
```{r sharpe-winning-model, fig.height=3.5, fig.width=9}
sw_model <- lm(win_pct ~ premium_surplus + non_premium_surplus, data = sw_panel)
sw_coefs <- broom::tidy(sw_model, conf.int = TRUE) %>%
  filter(term != "(Intercept)") %>%
  mutate(
    term_label = recode(term,
      premium_surplus = "Premium surplus\n(QB / OT / EDGE / WR)",
...
ggplot(sw_coefs, aes(x = estimate, y = reorder(term_label, estimate), color = sig)) +
  ...
  labs(
    title = "Premium-position surplus is what drives wins",
    subtitle = paste0("win% ~ premium surplus + non-premium surplus\n",
...
prem_row <- sw_coefs %>% filter(term == "premium_surplus")
np_row <- sw_coefs %>% filter(term == "non_premium_surplus")

prem_sig <- ifelse(prem_row$p.value < 0.05, "significant",
                   ifelse(prem_row$p.value < 0.10, "borderline", "not significant"))
np_sig   <- ifelse(np_row$p.value < 0.05, "significant",
                   ifelse(np_row$p.value < 0.10, "borderline", "not significant"))

cat(paste0(
  "**Premium surplus** (β = ", round(prem_row$estimate, 3),
  ", p = ", round(prem_row$p.value, 3), ") is ", prem_sig,
  " — each additional cap-percent point of surplus generated at QB/OT/EDGE/WR adds roughly ",
  sprintf("%.1f", prem_row$estimate * 100),
  " points of win percentage. **Non-premium surplus** (β = ", round(np_row$estimate, 3),
  ", p = ", round(np_row$p.value, 3), ") is ", np_sig,
  "; hitting on non-premium positions is nice, but it does not separate teams in the standings."
))
```
...
- **Realized surplus — actually hitting on picks at premium positions — is what translates to winning.** Teams in the top quintile of premium-position surplus win meaningfully more than teams in the bottom quintile, and the playoff-rate gap is even larger.
- **Premium surplus is where the signal lives.** Non-premium surplus is noisy in the win column; the wins come from hitting at QB, OT, EDGE, and WR.
- **Allocation alone doesn't predict anything.** Choosing the "right" positions by Sharpe adds zero to win probability unless those picks actually produce.
...
cat(paste0(
  ...
  "7. **Premium-position surplus actually translates to wins.** Across ", nrow(sw_panel),
  " team-seasons from 2014 to 2025, teams in the top quintile of premium-position surplus (QB/OT/EDGE/WR) win **",
  scales::percent(top$wp, accuracy = 0.1), "** and make the playoffs **",
  scales::percent(top$po, accuracy = 1), "** of the time; the bottom quintile wins **",
  scales::percent(bot$wp, accuracy = 0.1), "** and reaches the playoffs just **",
  scales::percent(bot$po, accuracy = 1), "**.\n\n",

  "8. **It's execution at premium positions, not allocation.** In the multivariate model, ",
  "premium surplus (β = ", round(prem_row$estimate, 3), ", p = ", round(prem_row$p.value, 3),
  ") carries the signal, while non-premium surplus (β = ", round(np_row$estimate, 3),
  ", p = ", round(np_row$p.value, 3), ") does not separate winners from losers. ",
  "The Sharpe framework identifies where expected value is highest; winning comes from realizing that value at QB, OT, EDGE, and WR."
))
```
...
position as interchangeable, and every team's roster as a blank slate.
...
make_player_gt <- function(data, title, subtitle = NULL) {
  tbl <- data %>%
    mutate(rank = row_number()) %>%
    rename(drafted_by = team) %>%
    mutate(
      drafted_by = dplyr::recode(drafted_by,
        "GNB"="GB","KAN"="KC","NOR"="NO","NWE"="NE","SFO"="SF","TAM"="TB",
        "OAK"="LV","LVR"="LV","SD"="LAC","SDG"="LAC","STL"="LAR","LA"="LAR")
    ) %>%
    select(rank, gsis_id, pfr_player_name, pos_group, drafted_by,
