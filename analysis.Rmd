...
### What About RB in the Top 5?

```{r rb-top5, results='asis'}
# All top-5 RBs in our dataset
top5_rb <- player_lookup %>% filter(pos_group == "RB", pick >= 2, pick <= 5, season <= 2022)

# All picks 2-5
picks_2_5 <- player_lookup %>% filter(pick >= 2, pick <= 5, season <= 2022)
picks_2_5_elite <- picks_2_5 %>% filter(player_return > elite_threshold)

cat(paste0(
  "Mock drafts have an RB going as high as **#3 overall** this year. Pick #3 falls in the ",
  "top-10 tier, where the Sharpe ratio for RB is **", round(rb_top10$sharpe_elite, 2),
  "** — the worst of any offensive position.\n\n",

  "What does -0.62 mean in practice? Here are the ", nrow(top5_rb),
  " RBs drafted at picks 2–5 since 2010:\n\n",

  "| Player | Pick | Year | Hit? | Player Return |\n",
  "|---|---|---|---|---|\n"
))

for (i in seq_len(nrow(top5_rb))) {
  cat(paste0(
    "| ", top5_rb$pfr_player_name[i],
    " | #", top5_rb$pick[i],
    " | ", top5_rb$season[i],
    " | ", ifelse(top5_rb$is_hit[i], "Yes", "No"),
    " | ", percent(top5_rb$player_return[i], accuracy = 0.1),
    " |\n"
  ))
}

cat(paste0(
  "\nThree of four became starters — the hit rate (",
  percent(mean(top5_rb$is_hit), accuracy = 1),
  ") is strong. But **none cracked the elite threshold** of ",
  percent(elite_threshold, accuracy = 0.1),
  ". The average return was just ", percent(mean(top5_rb$player_return), accuracy = 0.1),
  " — roughly a third of what a top-5 QB produces.\n\n",

  "For context, across *all* positions drafted at picks 2–5 since 2010, the selections that produced ",
  "elite returns were: ",
  paste(picks_2_5_elite$pfr_player_name, paste0("(", picks_2_5_elite$pos_group, ", ", picks_2_5_elite$season, ")"), collapse = ", "),
  ". Every one is a QB, EDGE, or OT — the positions where top-10 Sharpe is positive. ",
  "The opportunity cost of using a top-5 pick on RB is not hypothetical; it's structural."
))
```
