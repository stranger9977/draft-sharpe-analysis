...
early_r2_wr <- df_eligible %>% filter(pos_group == "WR", tier == "Round 2") %>%
  arrange(desc(player_return)) %>% slice_head(prop = 0.5) %>%
  summarise(med = median(player_return)) %>% pull(med)
...
cat(paste0(
  ...
  "is outpaced by a **high-end Round 2 WR** (median of the top half of R2 WR returns: ",
  round(early_r2_wr, 3), ", ~**$", round(early_r2_wr * 275), "M**). The *best* RB outcome in our sample doesn't match a *typical high-end* R2 WR. ",
