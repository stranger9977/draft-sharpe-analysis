...
How often does a draft pick become a starter? A "hit" means the player's average snap percentage over their first 4 NFL seasons reaches at least 75% of the positional baseline — the typical snap share for an established starter at that position. The baseline varies because positions share the field differently:

| Position | Starter Baseline | 75% = Hit Threshold |
|---|---|---|
| IOL | 99.6% | 74.7% |
| OT | 97.3% | 73.0% |
| S | 95.2% | 71.4% |
...
RBs have the lowest bar because even a bellcow RB1 shares snaps on passing downs and in rotation — 56% is what a starter looks like. IOL starters almost never come off the field, so their bar is the highest. Snap % is averaged over **all expected games** across 4 seasons, so missed games due to injury count as zeros — availability matters.
