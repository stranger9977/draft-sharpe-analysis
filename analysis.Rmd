...
sw_quartile_raw <- sw_panel %>%
  mutate(bin = dplyr::ntile(total_surplus, 4)) %>%
  group_by(bin) %>%
  summarise(
  ...
  mutate(
    bin_label = paste0(
      c("Bottom 25%", "Q2", "Q3", "Top 25%"),
      "\n(", round(min_s, 1), "–", round(max_s, 1), ")"
    )
  )

sw_quartile <- sw_quartile_raw %>%
...
ggplot(sw_quartile, aes(x = bin, y = value, color = metric, group = metric)) +
  ...
  scale_x_continuous(breaks = 1:4, labels = sw_quartile_raw$bin_label) +
  labs(
    title = "More draft surplus → more wins and more playoff berths",
    subtitle = paste0("Team-seasons bucketed into quartiles by rookie-deal surplus (drafts Y–4 to Y–1) | 2014–2025, n = ",
                      nrow(sw_panel)),
    x = "Quartile of rookie-deal surplus",
    y = NULL,
