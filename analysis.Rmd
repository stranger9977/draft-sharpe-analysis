...
```{r team-efficiency-later-rounds, fig.height=7, fig.width=12}
# For rounds 2-7, use RELATIVE Sharpe (above/below round average)
# so the bars have meaningful blue/red spread instead of all red
team_picks_all <- df_eligible %>%
  ...
  mutate(sharpe_elite = replace_na(sharpe_elite, 0)) %>%
  # Compute relative Sharpe within each tier
  group_by(tier) %>%
...
# For R2/R3/R4-7, compute average relative Sharpe per team
...
ggplot(team_later, aes(x = reorder(team, avg_relative), y = avg_relative)) +
  geom_col(aes(fill = avg_relative > 0), width = 0.7, show.legend = FALSE) +
  geom_nfl_wordmarks(aes(team_abbr = team), width = 0.055) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray40") +
  scale_fill_manual(values = c("TRUE" = "#2196F3", "FALSE" = "#F44336")) +
  facet_wrap(~draft_round, scales = "free_y") +
  coord_flip() +
  labs(
    title = "Positional Allocation vs. Round Average (2018–2022)",
    subtitle = "Blue = drafting above-average Sharpe positions for that round | Red = below | Min 3 picks per round",
