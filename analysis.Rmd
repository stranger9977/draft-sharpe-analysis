...
late first round and WR offers the best value on Day 2. RB shows negative returns across
...
wr_day2 <- sharpe %>% filter(pos_group == "WR", tier == "Day 2") %>% pull(sharpe_elite)
...
cat(paste0(
  ...
  round(best_late1$sharpe_elite, 2), "). Even a Day 2 WR (Sharpe: ",
  ...
  "If you need a running back, Day 2 or Day 3 Early gets you nearly equivalent production ",
...
red cells are where it doesn't. Most of the green is in the late first and Day 2 — not the
...
- **Timo Riske's hit rate methodology** ([PFF, 2022](https://www.pff.com/news/draft-pff-data-study-what-makes-an-nfl-draft-pick-a-hit)):
...
surplus_data <- df_eligible %>%
  ...
  filter(tier %in% c("Top 10", "Late 1st", "Day 2"))
...
How often does a draft pick become a starter? This heatmap shows hit rates
by position group and draft tier.
...
wr_day2 <- sharpe %>% filter(pos_group == "WR", tier == "Day 2")

best_rb_return <- df_eligible %>% filter(pos_group == "RB", tier == "Top 10") %>%
  slice_max(player_return, n = 1)
med_wr_day2 <- df_eligible %>% filter(pos_group == "WR", tier == "Day 2") %>%
...
cat(paste0(
  ...
  ") barely matches the **median Day 2 WR** (", round(med_wr_day2, 3), "). ",
  ...
  "**WR Day 2 Sharpe: ", round(wr_day2$sharpe_elite, 2), "**\n\n",
  ...
  round(max(df_eligible %>% filter(pos_group == "WR") %>% pull(second_apy_cap_pct)) * 100),
  "%. The upside is structurally capped.\n\n",
  "In this framework, a Day 2 WR looks like a substantially better risk-adjusted investment ",
...
ggplot(compare_data, aes(x = player_return, y = label, color = label)) +
  ...
  scale_color_manual(values = c("RB Top 10" = "#377EB8", "WR Day 2" = "#4DAF4A")) +
  scale_x_continuous(labels = percent_format()) +
  labs(
    title = "Top-10 RB vs Day 2 WR: Every Player's Return",
...
dc_plot <- sharpe %>%
  filter(pos_group %in% c("QB", "OT", "EDGE", "CB", "WR"),
         tier %in% c("Top 10", "Late 1st", "Day 2")) %>%
...
tiers <- sharpe %>%
  ...
  mutate(
    category = case_when(
      ...
      `Top 10` < 0 & (`Late 1st` > 0 | `Day 2` > 0) ~ "Wait — value is later",
...
if (nrow(for_better_late) > 0) {
  ...
  for (i in seq_len(nrow(for_better_late))) {
    ...
    if (row$pos_group == "OT") {
      ...
      cat(" — WR talent is deep enough that Day 2 picks (Sharpe: ",
          round(row$`Day 2`, 2), ") are competitive with the late first. ",
...
if (nrow(for_wait) > 0) {
  ...
  for (i in seq_len(nrow(for_wait))) {
    row <- for_wait[i, ]
    best_later <- max(row$`Late 1st`, row$`Day 2`, na.rm = TRUE)
    best_tier <- if_else(row$`Late 1st` >= row$`Day 2`, "Late 1st", "Day 2")
    cat(paste0("- **", row$pos_group, "**: Top 10 = ", round(row$`Top 10`, 2),
               " → ", best_tier, " = ", round(best_later, 2)))
    if (row$pos_group == "CB") {
      cat(" — CB is a boom-or-bust position in the top 10, but the late first and Day 2 ",
...
if (nrow(neg_all) > 0) {
  cat(paste0("\n### Negative everywhere in Round 1\n\n",
             ...
             "The data suggests Day 2, Day 3, or free agency as more efficient paths — though ",
...
wasted_top10 <- sharpe %>%
  ...
  filter(tier %in% c("Top 10", "Late 1st", "Day 2")) %>%
  pivot_wider(names_from = tier, values_from = sharpe_elite) %>%
  mutate(top10_is_best = `Top 10` >= pmax(`Late 1st`, `Day 2`, na.rm = TRUE)) %>%
...
best_late1 <- sharpe %>% filter(tier == "Late 1st", !is.na(sharpe_elite)) %>%
  slice_max(sharpe_elite, n = 1)
best_day2 <- sharpe %>% filter(tier == "Day 2", !is.na(sharpe_elite)) %>%
  slice_max(sharpe_elite, n = 1)
...
cat(paste0(
  ...
  "3. **WR value appears concentrated on Day 2.** The best WR Sharpe (",
  round(best_day2$sharpe_elite, 2), " on Day 2) suggests that comparable WR production is ",
  "available later in the draft. Players like DK Metcalf (pick 64) and Davante Adams (pick 53) ",
  "illustrate this, though Day 2 WRs also bust at higher rates.\n\n",
...
Methodology: [Riske/PFF hit rates](https://www.pff.com/news/draft-pff-data-study-what-makes-an-nfl-draft-pick-a-hit),
