...
Mock drafts have Jeremiah Love going as high as [**#4 overall**](https://grindingthemocks.shinyapps.io/Dashboard/). He's been mocked to RB-needy teams like the Titans, Cardinals, Commanders, and Giants — all teams with top-10 picks and needs outside of the running back position.
...
gtsave(r5_gt, filename = "charts/r5_hits_by_team.png", vwidth = 900)
cat("![Top 5 Teams by 5th-Round Hits](charts/r5_hits_by_team.png)\n\n")

cat("The teams that find the most late-round starters tend to be the same teams that aggressively trade back to accumulate extra picks — more lottery tickets means more chances to hit.")
```

## The Only Reliable Path to Elite Talent

For certain positions, the draft isn't just a good way to acquire talent — it's the *only* reliable way. The free agency market for premium positions like QB and OT is so thin that teams can't count on signing elite players when they need them.
...
cat(paste0("Only **", ot_n, " starter-caliber OTs** and **", qb_n, " QBs** hit free agency ",
           "per year — compare that to **", rb_n, " RBs** and **", iol_n, " IOL**. ",
...
cat(paste0(
  ...
  "The WR market inefficiency extends beyond the draft. Jakobi Meyers (UDFA), Romeo Doubs (4th round), ",
  "and Rashid Shaheed (UDFA) all signed contracts that would make them the highest-paid running back ",
  "on most rosters. The market is telling us something: even league-average wide receivers command ",
  "premium dollars, while elite running backs get paid like average starters at other positions. ",
  "That gap is the whole argument in one sentence.\n"
))
```
...
So how do teams actually behave? If the data says QB drives the most surplus, WR value is concentrated in Day 2, and RB replacement is cheap — are front offices listening?
...
cat(paste0(
  ...
  "**negative Sharpe in every first-round slot**.\n\n",
...
```{r eagles-case-study, results='asis'}
phi_picks <- df_eligible %>%
  filter(team == "PHI", season >= 2018) %>%
  mutate(round_num = case_when(
    pick <= 32 ~ 1, pick <= 64 ~ 2, pick <= 100 ~ 3, TRUE ~ 4
  ))

phi_r1 <- phi_picks %>% filter(round_num == 1)
phi_r1_pos <- phi_r1 %>% count(pos_group) %>% arrange(desc(n))

cat(paste0(
  "Philadelphia's Super Bowl LIX roster is the Sharpe framework in action. ",
  "Their first-round picks since 2018 have gone almost exclusively to premium positions: ",
  paste(paste0(phi_r1_pos$pos_group, " (", phi_r1_pos$n, ")"), collapse = ", "),
  ". Meanwhile, they built their skill-position depth through free agency and trades. ",
  "The Eagles didn't avoid running backs because they don't value the position; they avoided ",
  "*drafting* running backs early because the market lets you acquire them for less."
))
```

```{r eagles-drafted-table, results='asis'}
# Eagles drafted core — pull all PHI R1/R2 key picks from data
phi_drafted <- player_lookup %>%
  filter((team == "PHI" | grepl("PHI", team)),
         season >= 2018, season <= 2022,
         pick <= 64) %>%
  arrange(desc(player_return))

if (nrow(phi_drafted) > 0) {
  phi_d_gt <- phi_drafted %>%
    select(pfr_player_name, pos_group, season, pick, rookie_surplus, second_apy_cap_pct, player_return) %>%
    gt() %>%
    cols_label(
      pfr_player_name = "PLAYER", pos_group = "POS", season = "YEAR",
      pick = "PICK", rookie_surplus = "ROOKIE SURPLUS",
      second_apy_cap_pct = "2ND CONTRACT", player_return = "PLAYER RETURN"
    ) %>%
    ...
    tab_header(
      title = md("**Eagles Drafted Core (2018–2022)**"),
      subtitle = "Round 1 and 2 picks"
    ) %>%
    tab_style(style = cell_text(weight = "bold"), locations = cells_column_labels()) %>%
    tab_options(table.font.size = px(13), table.width = pct(100))

  gtsave(phi_d_gt, filename = "charts/eagles_drafted.png", vwidth = 900)
  cat("![Eagles Drafted Core](charts/eagles_drafted.png)\n\n")
}
```

```{r eagles-fa-table, results='asis'}
# Eagles FA acquisitions — skill positions acquired through trades/FA
phi_fa_names <- c("Saquon Barkley", "A.J. Brown")

phi_fa <- player_lookup %>%
  filter(grepl(paste(phi_fa_names, collapse = "|"), pfr_player_name)) %>%
  arrange(desc(player_return))
...
if (nrow(phi_fa) > 0) {
  phi_fa_gt <- phi_fa %>%
    ...
    fmt_percent(columns = c(rookie_surplus, second_apy_cap_pct, player_return), decimals = 1) %>%
    data_color(columns = player_return,
               palette = c("#f7f7f7", "#fde0dd", "#fa9fb5", "#c51b8a")) %>%
    tab_header(
      title = md("**Eagles FA / Trade Acquisitions**"),
      subtitle = "Skill positions acquired through the market — drafted by other teams"
    ) %>%
    tab_footnote(
      footnote = "Player return shown is from when they were originally drafted, not their Eagles tenure.",
      locations = cells_column_labels(columns = player_return)
    ) %>%
    tab_style(style = cell_text(weight = "bold"), locations = cells_column_labels()) %>%
    tab_options(table.font.size = px(13), table.width = pct(100))

  gtsave(phi_fa_gt, filename = "charts/eagles_fa.png", vwidth = 900)
  cat("![Eagles FA / Trade Acquisitions](charts/eagles_fa.png)\n\n")
}
```

```{r eagles-narrative, results='asis'}
hurts_pr <- phi_drafted %>% filter(grepl("Hurts", pfr_player_name)) %>% pull(player_return)
smith_pr <- phi_drafted %>% filter(grepl("Smith", pfr_player_name)) %>% pull(player_return)
mailata_pr <- phi_drafted %>% filter(grepl("Mailata", pfr_player_name)) %>% pull(player_return)
dickerson_pr <- phi_drafted %>% filter(grepl("Dickerson", pfr_player_name)) %>% pull(player_return)

cat(paste0(
  "Jalen Hurts was a late first-round QB hit with a massive player return (",
  percent(hurts_pr, accuracy = 0.1),
  ") — exactly the kind of outcome the Sharpe ratio says you should be swinging for. ",
  "DeVonta Smith was a top-10 WR hit (",
  percent(smith_pr, accuracy = 0.1),
  "). Jordan Mailata, taken at pick 233, was cost-controlled even on his second contract ",
  "as he took a little longer to develop — but he's now among the best left tackles in the league. ",
  "The return on where they got him is remarkable.\n\n",

  "Landon Dickerson as an IOL pick in Round 2 is technically a negative-Sharpe bet, but his player return (",
  percent(dickerson_pr, accuracy = 0.1),
  ") just misses the elite threshold — a case where the team's confidence in the player ",
  "overcoming the opportunity cost was justified.\n\n",

  "On the acquisition side, the Eagles had the rare opportunity to trade for A.J. Brown and sign him ",
  "to a second contract — an unusual channel for acquiring an elite player outside the draft. ",
  "The Barkley signing, in retrospect, looks like a steal given his role in the Super Bowl run. ",
  "Both acquisitions illustrate the strategy: draft premium positions, then use the market to fill ",
  "skill positions where the FA supply is deep."
))
```
...
## Appendix: Average Sharpe Ratio by Team — All Rounds Combined

```{r team-sharpe-all-rounds-combined, fig.height=11, fig.width=9}
# All picks, all rounds, with Sharpe joined
...
ggplot(team_allrounds, aes(x = reorder(team, avg_sharpe), y = avg_sharpe)) +
  geom_nfl_wordmarks(aes(team_abbr = team), width = 0.09) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray40") +
  coord_flip() +
  labs(
    title = "Average Sharpe Ratio by Team — All Rounds (2018–2022)",
    subtitle = "How well has each team allocated draft capital across the entire draft? | Last 5 eligible classes",
    x = NULL, y = "Average Sharpe of All Picks"
  ) +
  theme_minimal(base_size = 13)
```
