...
We sorted every team-season into quartiles by total rookie-deal surplus and plotted win% and playoff rate by quartile.
...
sw_cat_raw <- purrr::map_dfr(cat_specs$cat_col, function(col) {
  sw_panel %>%
    mutate(val = .data[[col]], bin = dplyr::ntile(val, 4)) %>%
    group_by(bin) %>%
    summarise(
...
ggplot(sw_cat, aes(x = bin, y = value, color = metric, group = metric)) +
  ...
  scale_color_manual(values = c("Win %" = "#08519c", "Playoff appearance rate" = "#b45f06"),
                     name = NULL) +
  scale_y_continuous(labels = scales::percent, limits = c(0.15, 0.75)) +
  scale_x_continuous(breaks = 1:4,
                     labels = function(b) {
                       c("Bottom 25%", "Q2", "Q3", "Top 25%")[b]
                     }) +
  facet_wrap(~ cat_label, nrow = 1) +
  labs(
    title = "Where does draft surplus actually translate to wins?",
    subtitle = paste0("Team-seasons bucketed into quartiles by category surplus (drafts Y–4 to Y–1) | 2014–2025, n = ",
                      nrow(sw_panel)),
    x = "Quartile of category surplus",
    y = NULL,
...
bot <- sw_panel %>% mutate(b = dplyr::ntile(total_surplus, 4)) %>% filter(b == 1) %>%
  summarise(wp = mean(win_pct), po = mean(made_po))
top <- sw_panel %>% mutate(b = dplyr::ntile(total_surplus, 4)) %>% filter(b == 4) %>%
  summarise(wp = mean(win_pct), po = mean(made_po))

cat(paste0(
  "The pattern is monotonic: teams in the **top quartile** of rookie-deal surplus ",
  "win **", scales::percent(top$wp, accuracy = 0.1),
  "** of their games and make the playoffs **", scales::percent(top$po, accuracy = 1),
  "** of the time. Teams in the **bottom quartile** win **",
  ...
  "-point** gap in win percentage and roughly a **", round(top$po / pmax(bot$po, 0.01), 1),
  "× gap** in playoff rate, purely as a function of how much surplus value a team's active ",
  "rookie class has generated."
))
```
...
cat(paste0(
  ...
  "7. **Draft surplus actually translates to wins.** Across ", nrow(sw_panel), " team-seasons from 2014 to 2025, ",
  "total rookie-deal surplus correlates with win % at **r = ",
  round(cor(sw_panel$total_surplus, sw_panel$win_pct), 2),
  "** (p < 0.0001). The top quartile of surplus wins **", scales::percent(top$wp, accuracy = 0.1),
  "** and reaches the playoffs **", scales::percent(top$po, accuracy = 1),
  "** of the time; the bottom quartile wins **", scales::percent(bot$wp, accuracy = 0.1),
  "** and reaches the playoffs just **", scales::percent(bot$po, accuracy = 1), "**.\n\n",
