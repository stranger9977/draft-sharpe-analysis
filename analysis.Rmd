...
phi_table <- phi_all %>%
  mutate(
    ...
    second_contract_team = "PHI"
  ) %>%
  select(pfr_player_name, pos_group, season, pick, channel,
         rookie_surplus, second_apy_cap_pct, second_contract_team, player_return)
...
phi_gt <- phi_table %>%
  ...
  cols_label(
    ...
    second_contract_team = "2ND CONTRACT TEAM",
    player_return = "PLAYER RETURN"
  ) %>%
...
# Look up the team that signed each player's second contract
contracts_raw <- nflreadr::load_contracts()

second_contract_team <- contracts_raw %>%
  filter(!is.na(gsis_id), !is.na(draft_year)) %>%
  mutate(yrs_since_draft = year_signed - draft_year) %>%
  # 2nd contract is the first contract signed after the rookie deal (~4 yrs after draft)
...
rd1 <- player_lookup %>%
  ...
  arrange(desc(player_return)) %>%
  head(25) %>%
  left_join(second_contract_team, by = "gsis_id") %>%
  mutate(second_contract_team = ifelse(is.na(second_contract_team), "—", second_contract_team))
...
rd1_gt <- rd1_tbl %>%
  ...
  gt_nfl_headshots(columns = "gsis_id", height = 35) %>%
  gt_nfl_wordmarks(columns = "team", height = 25) %>%
  gt_nfl_wordmarks(columns = "second_contract_team", height = 25) %>%
  cols_label(
    rank = "", gsis_id = "",
    pfr_player_name = "PLAYER", pos_group = "POS",
    team = "DRAFTED BY", second_contract_team = "2ND CONTRACT",
    season = "YEAR", pick = "PICK",
    rookie_surplus = "ROOKIE SURPLUS",
    second_apy_cap_pct = "2ND CONTRACT %",
    player_return = "PLAYER RETURN"
  ) %>%
  fmt_percent(columns = c(rookie_surplus, second_apy_cap_pct, player_return), decimals = 1) %>%
  data_color(columns = player_return,
             palette = c("#f7f7f7", "#c6dbef", "#4292c6", "#08519c"), domain = NULL) %>%
  data_color(columns = rookie_surplus,
             palette = c("#f7f7f7", "#c7e9c0", "#41ab5d", "#006d2c"), domain = NULL) %>%
  data_color(columns = second_apy_cap_pct,
             palette = c("#f7f7f7", "#dadaeb", "#9e9ac8", "#6a51a3"), domain = NULL) %>%
  tab_header(
  ...
  tab_style(style = cell_text(weight = "bold"), locations = cells_column_labels()) %>%
  tab_options(table.font.size = px(13), column_labels.font.size = px(11),
              data_row.padding = px(4))

gtsave(rd1_gt, filename = "charts/player_lookup_rd1.png", vwidth = 1000)
