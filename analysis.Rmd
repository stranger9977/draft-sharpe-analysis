...
cat(paste0(
  "**Probably not.** RB has a Sharpe of **", round(rb_top10$sharpe_elite, 2),
...
```{r team-efficiency-data}
# Load team info for colors/names
teams <- load_teams()

# For ALL picks, get the Sharpe of that position x tier
team_picks_all <- df_eligible %>%
  ...
  mutate(
    sharpe_elite = replace_na(sharpe_elite, 0),
    ...
    draft_round = factor(draft_round, levels = c("Round 1", "Round 2", "Round 3", "Rounds 4-7"))
  ) %>%
  filter(season >= 2018)

# Average Sharpe per team per round
team_by_round <- team_picks_all %>%
...
# Overall R1 for ordering
team_r1_order <- team_by_round %>%
  filter(draft_round == "Round 1") %>%
  arrange(avg_sharpe) %>%
  pull(team)
# Add teams that don't appear in R1
all_teams_r <- unique(team_by_round$team)
team_r1_order <- c(setdiff(all_teams_r, team_r1_order), team_r1_order)
```

```{r team-efficiency-faceted, fig.height=11, fig.width=14}
team_by_round %>%
  mutate(team = factor(team, levels = team_r1_order)) %>%
  ggplot(aes(x = team, y = avg_sharpe)) +
  geom_col(aes(fill = avg_sharpe > 0), width = 0.7, show.legend = FALSE) +
  geom_nfl_wordmarks(aes(team_abbr = team), width = 0.055) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray40") +
  scale_fill_manual(values = c("TRUE" = "#2196F3", "FALSE" = "#F44336")) +
  facet_wrap(~draft_round, nrow = 2, scales = "free_x") +
  coord_flip() +
  labs(
    title = "Average Sharpe Ratio by Team & Draft Round (2018–2022)",
    subtitle = "Blue = positive Sharpe (good allocation) | Red = negative (would've been better off signing FAs) | Min 2 picks per round",
    x = NULL, y = "Average Sharpe of Picks"
  ) +
...
# Find best/worst R1 teams
r1_data <- team_by_round %>% filter(draft_round == "Round 1")
best_r1 <- r1_data %>% slice_max(avg_sharpe, n = 3)
worst_r1 <- r1_data %>% slice_min(avg_sharpe, n = 3)

# Find teams that are good in later rounds but bad in R1 (or vice versa)
late_round <- team_by_round %>% filter(draft_round %in% c("Round 2", "Round 3"))
late_avg <- late_round %>% group_by(team) %>% summarise(late_sharpe = mean(avg_sharpe), .groups = "drop")
r1_avg <- r1_data %>% select(team, r1_sharpe = avg_sharpe)
divergent <- r1_avg %>%
  inner_join(late_avg, by = "team") %>%
  mutate(gap = late_sharpe - r1_sharpe)

best_late_worst_r1 <- divergent %>% filter(r1_sharpe < 0) %>% slice_max(gap, n = 1)
worst_late_best_r1 <- divergent %>% filter(r1_sharpe > 0) %>% slice_min(gap, n = 1)

# Count how many teams are negative in R4-7
r47 <- team_by_round %>% filter(draft_round == "Rounds 4-7")
r47_neg_pct <- mean(r47$avg_sharpe < 0)

cat(paste0(
  "**Round 1 is where strategy matters most.** The spread between the best and worst teams is enormous — ",
  best_r1$team[1], " averages +", round(best_r1$avg_sharpe[1], 2),
  " per first-round pick while ", worst_r1$team[1], " sits at ",
  round(worst_r1$avg_sharpe[1], 2), ". That's not luck over 5 drafts; it's positional philosophy.\n\n",

  "**The later rounds converge.** By Rounds 4–7, ", percent(r47_neg_pct, accuracy = 1),
  " of teams are in negative territory. This isn't a talent evaluation failure — it's just the math. ",
  "Late-round picks rarely produce elite returns at *any* position, so almost every pick has negative Sharpe. ",
  "The lesson: late-round capital is nearly fungible. The real differentiation happens in the first two rounds.\n\n",

  "**Watch for the divergers.** Some teams draft well in Round 1 but struggle to find value in Rounds 2–3, ",
  "and vice versa. ", best_late_worst_r1$team[1], " is a good example — negative in Round 1 but ",
  "relatively better in the middle rounds, suggesting their positional targeting improves when they're not ",
  "reaching for need at the top of the draft."
))
```

```{r team-position-strategy-recent, fig.height=10, fig.width=10}
# R1 position strategy (stacked bar)
...
recent_picks <- team_picks %>% filter(season >= 2018)
...
## The Draft Curve: How Teams Draft vs. How They Should

The team-level charts above show *which* teams get it right. But what about the league as a whole? If every GM followed the Sharpe data, how different would the draft look?

```{r draft-curve-data}
# Actual positional allocation by tier
actual_alloc <- df_eligible %>%
  filter(pos_group %in% key_positions) %>%
  count(tier, pos_group) %>%
  group_by(tier) %>%
  mutate(actual_pct = n / sum(n)) %>%
  ungroup()

# Join Sharpe to get "should" signal
curve_data <- actual_alloc %>%
  left_join(
    sharpe %>% select(pos_group, tier, sharpe_elite),
    by = c("pos_group", "tier")
  ) %>%
  mutate(
    sharpe_sign = ifelse(sharpe_elite >= 0, "Positive Sharpe", "Negative Sharpe"),
    pos_group = factor(pos_group, levels = key_positions)
  )
```

```{r draft-curve-actual-vs-optimal, fig.height=8, fig.width=11}
# Build "optimal" allocation: redistribute picks proportionally to positive-Sharpe positions
# Negative-Sharpe positions get 0%; positive ones share proportionally to their Sharpe
optimal <- sharpe %>%
  filter(pos_group %in% key_positions) %>%
  mutate(
    tier = factor(tier, levels = tier_order),
    weight = pmax(sharpe_elite, 0)
  ) %>%
  group_by(tier) %>%
  mutate(optimal_pct = weight / sum(weight)) %>%
  ungroup() %>%
  select(pos_group, tier, optimal_pct) %>%
  mutate(optimal_pct = replace_na(optimal_pct, 0))

comparison <- curve_data %>%
  left_join(optimal, by = c("pos_group", "tier")) %>%
  select(tier, pos_group, actual_pct, optimal_pct) %>%
  pivot_longer(cols = c(actual_pct, optimal_pct), names_to = "type", values_to = "pct") %>%
  mutate(
    type = ifelse(type == "actual_pct", "How Teams Actually Draft", "Sharpe-Optimal Allocation"),
    pos_group = factor(pos_group, levels = key_positions)
  )

ggplot(comparison, aes(x = pos_group, y = pct, fill = type)) +
  geom_col(position = "dodge", width = 0.7) +
  facet_wrap(~tier, nrow = 1) +
  scale_y_continuous(labels = percent_format()) +
  scale_fill_manual(values = c(
    "How Teams Actually Draft" = "#78909C",
    "Sharpe-Optimal Allocation" = "#2196F3"
  )) +
  labs(
    title = "How the NFL Drafts vs. How Sharpe Says It Should",
    subtitle = "Actual positional allocation vs. Sharpe-optimal allocation | Key positions, 2010–2022",
    x = NULL, y = "% of Picks", fill = NULL
  ) +
  theme_minimal(base_size = 13) +
  theme(
    legend.position = "top",
...
```{r draft-curve-waste, fig.height=5, fig.width=8}
# What % of picks at each tier go to negative-Sharpe positions?
waste_by_tier <- df_eligible %>%
  left_join(
    sharpe %>% select(pos_group, tier, sharpe_elite),
    by = c("pos_group", "tier")
  ) %>%
  ...
  summarise(
    total = n(),
    neg_sharpe = sum(sharpe_elite < 0, na.rm = TRUE),
    neg_pct = neg_sharpe / total,
    .groups = "drop"
  ) %>%
  mutate(tier = factor(tier, levels = tier_order))

ggplot(waste_by_tier, aes(x = tier, y = neg_pct)) +
  geom_col(fill = "#F44336", width = 0.6, alpha = 0.85) +
  geom_text(aes(label = percent(neg_pct, accuracy = 1)), vjust = -0.5, fontface = "bold", size = 5) +
  scale_y_continuous(labels = percent_format(), limits = c(0, 1.08), expand = c(0, 0)) +
  labs(
    title = "Wasted Draft Capital: Picks Spent on Negative-Sharpe Positions",
    subtitle = "% of picks at each tier that go to positions where Sharpe says 'just sign a free agent'",
    x = NULL, y = "% of Picks with Negative Sharpe"
  ) +
  theme_minimal(base_size = 14) +
...
top10_waste <- waste_by_tier %>% filter(tier == "Top 10")
r1_waste <- waste_by_tier %>% filter(tier %in% c("Top 10", "Late 1st"))
r1_total_neg <- sum(r1_waste$neg_sharpe)
r1_total <- sum(r1_waste$total)

cat(paste0(
  "**The gap is real.** In the top 10, ", percent(top10_waste$neg_pct, accuracy = 1),
  " of picks go to positions where the Sharpe ratio is negative — positions where the data says ",
  "you'd be better off signing a free agent. Across all of Round 1, that's **",
  r1_total_neg, " of ", r1_total, " picks** (",
  percent(r1_total_neg / r1_total, accuracy = 1),
  ") spent on bets that historically don't pay off relative to the risk.\n\n",
  "By Round 2, it's over 75%. By Rounds 4–7, it's essentially 100% — which makes sense. ",
  "Late-round picks rarely produce elite returns at any position, so the Sharpe is negative ",
  "almost everywhere. The actionable insight is in the first round: that's where teams have ",
  "real choices, and where the data shows the most room for improvement."
))
```
