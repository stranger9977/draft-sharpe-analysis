...
cat(paste0(
  ...
  "a savings of just ~$2–3M/yr. Ja'Marr Chase (pick 5, WR) earned ~4.1%, but a replacement WR costs ~10% — ",
  "a savings of ~$15M/yr. Both were legitimate hits on the field, but over four years Chase's rookie surplus ",
  "dwarfs Pitts' purely because WR has a more expensive free-agent market than TE. Same draft, adjacent picks, ",
  "similar production profiles early on — wildly different surplus value, because the cost-controlled advantage ",
  "at a premium position is structurally larger."
))
```
...
make_player_gt <- function(data, title, subtitle = NULL) {
  tbl <- data %>%
    ...
    select(rank, gsis_id, pfr_player_name, pos_group, drafted_by, second_team,
